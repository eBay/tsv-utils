...
help:
	...
	@echo 'LDC_LTO_RUNTIME - Turns on LTO for the runtime libraries (phobos, druntime). This uses'
	@echo '    LTO support built into the libraries shipped with LDC. Turn on with LDC_LTO_RUNTIME=1.'
	@echo '    LDC 1.9 or later required. Use LDC_BUILD_RUNTIME with early LDC releases.'
	@echo 'LDC_BUILD_RUNTIME - Turn on LTO for the runtime libraries by downloading the correct'
	@echo '    library source code and building. Turn on with LDC_BUILD_RUNTIME=1. LDC 1.5 or later'
	@echo '    required. When using LDC 1.9 or later LDC_LTO_RUNTIME is recommended instead.'
	...
	@echo "LDC_PGO_TYPE - Either 'IR' or 'AST'. Defaults to AST, and currently only AST is supported."
	@echo '    IR-PGO is anticipated in a future LDC release, but is not supported yet.'
	@echo ''
