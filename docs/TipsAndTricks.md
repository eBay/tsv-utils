...
An occasional task: Add a column to a TSV data stream. Same value for all records, but a custom header. There are any number of ways to do this, the best is the one you can remember. Here's a trick for doing this using the [tsv-filter](tool_reference/tsv-filter.md) `--label` option:
```
$ # The file
$ tsv-pretty data.tsv
 id  color   count
