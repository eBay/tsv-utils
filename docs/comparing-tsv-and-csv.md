...
By contrast, parsing TSV data is simple. Records can be read using the typical `readline` routines found in most programming languages. The fields in each record can be found using `split` routines. No special parser is needed. This is much more reliable. It is also faster, no CPU time is used parsing the escape syntax. The speed advantages are especially pronounced for record oriented operations, as fast line-oriented implementations can be used. For example, record counts (`wc -l`), deduplication (`uniq`, [tsv-uniq](ToolReference.md#tsv-uniq-reference)), sampling and shuffling (GNU `shuf`, [tsv-sample](ToolReference.md#tsv-sample-reference)).
