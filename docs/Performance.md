...
The specialty tool-kits have been anonymized in the descriptions below, except in the csv-to-tsv test, where two of these tool-kits were the best performers. (The purpose of these benchmarks is to gauge performance of the D tools, not to make comparisons between tool-kits.) Tool-kits used are from the set listed under [Other toolkits](../README.md#other-toolkits) in the README file. Python tools were not benchmarked, this would be a useful comparison to add. Tools that run in in-memory environments like R were excluded from the benchmarks.
...
| Tool                   | Time (seconds) | Max Memory (kbytes) |
| ---------------------- |--------------: | ------------------: |
| **tsv-filter**         |                |                     |
| mawk (M. Brennan Awk)  |                |                     |
| GNU awk                |                |                     |
