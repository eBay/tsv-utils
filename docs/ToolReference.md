...
* Line order randomization (_default behavior_): All input lines are output in a random order (aka. "shuffling"). All orderings are equally likely. Use `--n|num` to limit the sample size and produce a randomly ordered subsample. This is a form of simple random sampling. It is also known as "shuffling".
