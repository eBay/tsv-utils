...
* Shuffling - This is the default mode of operation. All lines are read into memory and written out in a random order. All orderings are equally likely.
...
* Sampling with replacement (`--r|replace`, `--n|num N`) - All lines are read into memory, then lines are randomly selected one at a time and written out. Lines can be selected multiple times. Output continues until `N` samples have been output.
